...
const AppProvider = ({ children }) => {
  ...
  const login = async (currentUser) => {
    ...
    try {
      const {
        data: { token, user },
      } = await customAxios.post(`auth/login/`, currentUser);
  ...
  const loadMoreProfilePosts = async () => {
    ...
    try {
      ...
      dispatch({
        type: LOAD_MORE_PROFILE_POSTS_SUCCESS,
        payload: data,
      });
    } catch (error) {
      console.log(error);
  ...
  const togglePostModal = async (postId, hide) => {
    ...
    if (hide === true) {
      ...
      return dispatch({
        ...
        payload: { post: {}, showPostModal: false, showOptionModal: false },
      });
    }
    ...
    if (state.posts.length === 0 && state.profilePosts.length === 0) {
      const {
        data: { post },
      } = await authFetch(`post/${postId}/`);
      dispatch({
        type: TOGGLE_POST_MODAL,
        payload: { post, showPostModal: true },
      });
    ...
    if (Object.keys(state.profileUser).length === 0) {
      ...
      dispatch({
        type: TOGGLE_POST_MODAL,
        payload: { postId, post, showPostModal: true },
      });
      await getFollowCondition(post.user.username);
    }
  ...
  const toggleFollowCondition = async (profileName) => {
    ...
    try {
      ...
      dispatch({
        type: CHANGE_FOLLOW_CONDITION_SUCCESS,
        payload: { isFollow },
      });
    } catch (error) {}
  };
  ...
  const addPost = async (newPost, body) => {
    const url = `posts/`;
    try {
      // true means empty formData
      if (newPost.entries().next().done) {
        // image under is uploaded file of nodejs/uploads folder
        await authFetch.post(url, { ...body, image: "/uploads/default.jpg" });
      } else {
        const {
          data: { image },
        } = await authFetch.post(`${url}/uploads/`, newPost);
        await authFetch.post(url, { ...body, image });
      }
  ...
  const editPost = async (body) => {
    const {status, post:postId} = body
    const url = `posts/${postId}/`;
    try {
      await authFetch.patch(url, body);
  ...
  const updateAvatar = async (avatar) => {
    const url = `profile/${state.profileUser.username}/`;
    try {
      const {
        data: { image },
      } = await authFetch.post(`profile/uploads/`, avatar);

      const {
        data: { user },
      } = await authFetch.patch(url, { avatar: image });
